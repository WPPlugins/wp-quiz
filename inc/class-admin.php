...
class WP_Quiz_Admin {
	...
	public function register_pages() {
		...
		$page_hook = add_submenu_page( 
			...
			__( 'Get Support for WP Quiz', 'wp-quiz' ), 
			__( 'Support', 'wp-quiz' ), 
			'manage_options', 
			'wp_quiz_support', 
			array( 'WP_Quiz_Page_Support', 
				'page' 
			)
		);
	...
	public function enqueue_scripts( $hook ) {
		...
		if ( in_array( $hook, array( 'post-new.php', 'post.php' ) ) ) {
			...
			wp_enqueue_style( 'wp_quiz', WP_QUIZ_ASSETS_URL . 'css/new-quiz.css', array(), WP_QUIZ_VERSION );
			...
			wp_enqueue_script( 'wp_quiz-react', WP_QUIZ_ASSETS_URL . 'js/content.min.js', array( 'jquery', 'semantic-checkbox-js', 'chosen-js', 'wp-color-picker' ), WP_QUIZ_VERSION, true );
			...
			wp_localize_script( 'wp_quiz-react', 'wq_l10n', array(
				...
				'fbnameNotice'			 => esc_html__( 'Possible name substiution (%%userfirstname%% = user first name, %%lastfirstname%% = user last name, %%friendfirstname%% = friend first name, %%friendfirstname%% = friend last name)', 'wp-quiz' ),
